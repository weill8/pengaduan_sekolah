...
@section('content')
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-8">
        <div>
            <div class="flex items-center gap-2 mb-1">
                ...
                <h1 class="text-xl font-bold text-slate-800 tracking-tight">Manajemen Kategori</h1>
            </div>
    ...
    {{-- Search Bar --}}
    <form method="GET" action="{{ route('admin.kategori.index') }}" class="mb-5">
        <div class="relative">
            ...
            <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari nama kategori..."
                class="w-full bg-white border border-slate-200 rounded-xl pl-10 pr-10 py-2.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/30 focus:border-indigo-400 transition-all shadow-sm">
            @if (request('search'))
    ...
    <div class="rounded-2xl border border-slate-100 overflow-hidden shadow-sm">
        ...
        <div class="bg-slate-50/80 px-6 py-3.5 border-b border-slate-100 flex items-center gap-2">
            ...
            <span class="text-xs font-bold text-slate-500 uppercase tracking-wider">Daftar Kategori</span>
        </div>
            ...
            <form method="POST" action="{{ route('admin.kategori.store') }}">
                ...
                <div class="px-6 py-5">
                    <label class="block text-xs font-bold text-slate-600 uppercase tracking-wider mb-2">Nama
                        Kategori</label>
                    <input type="text" name="ket_kategori" value="{{ old('ket_kategori') }}" maxlength="50"
                        autofocus
    ...
    {{-- Scripts --}}
    <script>
        function openModalEdit(id, ketKategori) {
            const baseUrl = "{{ url('admin/kategori') }}";
            document.getElementById('formEdit').action = `${baseUrl}/${id}`;
            document.getElementById('editKetKategori').value = ketKategori;
